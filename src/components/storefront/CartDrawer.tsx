  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-900/60 backdrop-blur-xs">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex w-full justify-end pointer-events-none">
        <div className="w-full max-w-md bg-white shadow-2xl flex flex-col h-full pointer-events-auto overflow-hidden">
          {/* Header */}
          <div className="p-4 sm:p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <ShoppingCart className="w-5 h-5 text-slate-900 shrink-0" />
          ...
          {/* Cart Items List */}
          <div className="flex-1 overflow-y-auto overflow-x-hidden p-3.5 sm:p-6 divide-y divide-slate-100">
            {items.length === 0 ? (
              ...
              items.map(item => {
                ...
                return (
                  <div key={item.product.id} className="py-3.5 sm:py-4 flex gap-3 min-w-0 max-w-full">
                    <img
                      src={img}
                      alt={item.product.name}
                      className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl object-cover bg-slate-100 border border-slate-200/80 shrink-0"
                    />
                    <div className="flex-1 flex flex-col justify-between min-w-0">
                      <div className="flex items-start justify-between gap-1.5">
                        <h4 className="text-xs font-semibold text-slate-900 line-clamp-2 leading-tight min-w-0 break-words">
                          {item.product.name}
                        </h4>
                        <button
                      ...
                      <div className="flex items-center justify-between pt-1.5 gap-2 flex-wrap">
                        ...
                        <div className="flex items-center border border-slate-200 rounded-lg overflow-hidden shrink-0 bg-slate-50">
                          <button
                            ...
                            className="p-1 sm:p-1.5 text-slate-600 hover:bg-slate-200 transition cursor-pointer"
                          >
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="px-2 text-xs font-semibold text-slate-900 min-w-[20px] text-center">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
          ...
          {items.length > 0 && (
            <div className="p-4 sm:p-6 border-t border-slate-100 bg-slate-50 space-y-3 sm:space-y-4 shrink-0">

...
export const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (user && user.is_email_verified) {
      api.getMerchantBusiness()
        .then(({ business, store }) => {
          setStoreData({
            storeSlug: store?.slug || 'chi-beauty',
        ...
        .catch(() => {
          setStoreData({
            storeSlug: 'chi-beauty',
            businessName: 'Chi Beauty & Glow',
            planName: 'Starter'
          });
        });
    }
  ...
  const storeUrl = `${window.location.origin}/${storeData?.storeSlug || 'chi-beauty'}`;
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row pb-16 md:pb-0">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 shrink-0">
        ...
        <div className="p-4 mx-3 my-3 bg-slate-50 rounded-xl border border-slate-200/80">
          <p className="text-sm font-semibold text-slate-900 truncate">{storeData?.businessName || 'Loading...'}</p>
          <div className="mt-3 flex items-center gap-2">
            <a
              href={`/${storeData?.storeSlug || 'chi-beauty'}`}
              target="_blank"
              rel="noreferrer"
        ...
        {/* Demo Switcher & User Footer */}
        <div className="p-4 border-t border-slate-200 space-y-3">
          <div className="bg-slate-100 p-2.5 rounded-xl text-xs space-y-1.5">
            <span className="font-semibold text-slate-600 block">Preview Identity:</span>
            <div className="flex gap-1.5">
              <button
                onClick={() => switchDemoRole('merchant')}
                className={`flex-1 py-1 rounded text-2xs font-medium transition cursor-pointer ${
                  !user?.is_platform_admin ? 'bg-white text-blue-700 shadow-2xs' : 'text-slate-600 hover:bg-white/50'
                }`}
              >
                Merchant
              </button>
              <button
                ...
                className={`flex-1 py-1 rounded text-2xs font-medium transition cursor-pointer ${
                  user?.is_platform_admin ? 'bg-white text-purple-700 shadow-2xs' : 'text-slate-600 hover:bg-white/50'
                }`}
              >
                Admin
              </button>
